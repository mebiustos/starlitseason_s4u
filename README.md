...
また、作者以外の環境で動くかどうかはわかりません。特にカメラ関連はもしかしたらPCにコントローラー（PAD）が接続されていないとターゲット指定が動作しない【かも】しれません。私用に作成したので許してほしい。

このドキュメントも適宜追記していこうと思っていますが、力尽きても許してほしい。
...
1. スターリットシーズンを起動し、Starge For You!のライブをSTARTしてください。
2. START実行から「Now Loading...」が終了するまでの間に、曲マクロ（READY）を割り当てたマウスのサイド１ボタンを押します。
3. これで、「Now Loading...」が終了し、画面中央のカラーコードが黒#000000以外になった時点からタイマーが開始され指定された経過時間に指定された操作が実行されます。
4. START実行前にマウスサイド１を押してしまうと、マウスポインタも動かない状態になり何もできなくなります。その場合はALT+TABで別のアプリケーションをアクティブにすれば「利用前の下準備 手順5」の設定により曲マクロ処理は強制的に終了します。

### 簡易記録機能

1. スターリットシーズンを起動し、Starge For You!のライブをSTARTしてください。
2. START実行から「Now Loading...」が終了するまでの間に、記録マクロが割り当てられているマウスのサイド２ボタンを押します。
3. ライブ中に押されたキーボードに対応するC#コードがKeyToKeyのコンソールに表示されます。
4. 記録が終わったらEscapeキーを押すと記録マクロは終了します。その際、記録された全てのC#コードはクリップボードにコピーされます。
5. これを適宜曲マクロのC#コードに貼り付けて、利用できます。
6. カメラの切り替えタイミングなどは適当にPキーなどを押してタイミングだけ参考にして、コードを自前で記述してください。
7. ちなみにこの記録機能は曲マクロにも実装されていますが、曲マクロは最後のコマンドが実行されたら即終了してしまうので適宜良いように使いわけてください。

## 制限事項
曲マクロの操作は記述された順番に実行されます。
経過時間でソートされたりはしません。
```
s4u.ETap(18000, Keys.NumPad1);
...
```
カメラのターゲット切り替え後の内部的なWAIT時間です。デフォルトは25ミリ秒です。
カメラのターゲット切り替えが不安定な場合はこの秒数を大きくすると解決すると思います。

```
s4u.SetCamGlobalTargetLeadMs(100);
```
本コマンド以降、以下の形式で指定されたカメラ動作は、ターゲットを指定する操作のみ指定された経過秒数より指定されたミリ秒だけ早く実行するようになります。
たとえば上記のコマンド例以降にs4u.Cam(13000, CameraTarget.No2, CameraType.F_アップ_足から頭へ)というコマンドが記述されていた場合、12900にターゲットを切り替える動作がに実行され、13000に実際のカメラ動作（F_アップ_足から頭へ）が実行されます。
