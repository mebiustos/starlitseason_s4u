...
13. （オプション・かなり推奨）KeyToKeyのメインウィンドウから「コンソール＞表示」を選択しコンソールウィンドウを表示し、さらに「コンソール＞常に最前面に表示」を選択してコンソールが常に最前面に表示されるように設定してください。
    ...
    ただしコンソールウィンドウが画面の中央に被らないよう注意してください。左下あたりに小さく表示しておくとよいです。邪魔な場合は最小化（非表示ではない）してください。
...
1. KeyToKeyのコンソールウィンドウが画面の中央に被っていないことを確認してください。画面の中央はローディング終了判定の為にカラーコードを監視しています。
