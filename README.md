...
13. （オプション・かなり推奨）KeyToKeyのメインウィンドウから「コンソール＞表示」を選択しコンソールウィンドウを表示し、さらに「コンソール＞常に最前面に表示」を選択してコンソールが常に最前面に表示されるように設定してください。
    ...
    左下あたりに小さく表示しておくとよいです。邪魔な場合は最小化してください。（非表示ではない）
...
3. これで、「Now Loading...」が終了し、画面中央のカラーコードが黒#000000以外になった時点からタイマーが開始され指定された経過時間に指定されたコマンドが実行されます。
...
```
スターリットシーズンのMVの仕様上、カメラのターゲット切り替えの後にだけは必ず内部的にWAITを入れる必要があり、その内部的なWAIT時間を指定できます。デフォルトは25ミリ秒です。カメラのターゲット切り替えが不安定な場合はマクロの先頭付近でこの秒数を大きくすると解決すると思います。もしくはSetCamGlobalTargetLeadMsコマンドを利用してください。
