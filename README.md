...
1. 曲マクロファイルの先頭付近に記述された「#define ライブ」という記述を見つけ、その先頭に//を追加して保存（Ctrl+S）します。これにより、ライブ演出マクロ部分が無効化され、かわりに演出スロット復元コード部が有効化されます。

   ![演出復元1-img1](https://imgur.com/zgpfohE.jpg)
   
2. 反映
3. STAGE FOR YOU!のメイン画面で曲マクロを割り当てたマウスのサイド１ボタンを押します。
4. 復元される
5. STAGE FOR YOU!のメイン画面に自動で戻
6. defineをもとに戻す＆セーブ
7. 反映
